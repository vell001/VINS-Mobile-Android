...
void vinsPnP::updateFeatures(vector<IMG_MSG_LOCAL> &feature_msg) {
    for (int i = 0; i < frame_count && i < features->size(); i++) {
        int j = 0;
...
void vinsPnP::solve_ceres() {
    ...
    for (int i = start_index; i < PNP_SIZE; i++) {
        ...
        problem.AddResidualBlock(imu_factor, NULL, para_Pose[i], para_Speed[i], para_Bias[i],
                                 para_Pose[j], para_Speed[j], para_Bias[j]);
    }
...
void vinsPnP::slideWindow() {
    if (frame_count == PNP_SIZE) {
        ...
        pre_integrations[PNP_SIZE] = new IntegrationBase{acc_0, gyr_0, Bas[PNP_SIZE],
                                                         Bgs[PNP_SIZE]};
